...
def download_wordlist():
    try:
        ...
        response = requests.get(hash_url, timeout=10) if requests.get(hash_url, timeout=10).status_code == 200 else requests.get(git_url, timeout=10)
        with open(main_wordlist, 'wb') as f:
            f.write(response.content)
        logger.info("Wordlist downloaded successfully.")
    except Exception as e:
        logger.error(f"Failed to download wordlist: {e}")
        print("Failed to download wordlist. Please check the logs.")
...
def configure_proxy(proxy_file):
    if not os.path.exists(proxy_file):
        ...
        logger.info("Proxy configuration saved successfully.")
    else:
        logger.info("Proxy configuration already exists.")
...
def configure_telegram_bot(bot_telegram_file):
    if not os.path.exists(bot_telegram_file):
        ...
        logger.info("Telegram bot configuration saved successfully.")
    else:
        logger.info("Telegram bot configuration already exists.")
...
def get_links(value):
    ...
    try:
        ...
        logger.error(f"Request failed for {value}: {e}")
        with open(error_file, "a") as input_to:
            input_to.write(value + "\n")
        return "skip"
...
def start_processing():
    ...
    batch_size = 10  # Reduced batch size to prevent being blocked
    for i in range(0, len(urls_list), batch_size):
        ...
        time.sleep(1)  # Adding a 1-second sleep per batch
